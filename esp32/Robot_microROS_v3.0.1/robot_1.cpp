// ESP32: Robot1_microROS_v3.0_Arduino.cpp
// Updated for Arduino motor controller with encoders and MPU6050
// Robot 1 with LD19 LiDAR and Arduino integration
...
void cmd_vel_callback(const void * msgin) {
    ...
    if (abs(left_pwm) < 10 && abs(right_pwm) < 10) {
        ARDUINO_SERIAL.println("S0");
    } else {
        ARDUINO_SERIAL.print("D");
        ARDUINO_SERIAL.print(left_pwm);
        ARDUINO_SERIAL.print(",");
        ARDUINO_SERIAL.println(right_pwm);
    }
}
...
void setup() {
    ...
    Serial.println("Robot 1 initialized with Arduino motor controller");
}
